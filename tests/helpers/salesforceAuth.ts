...
export interface JwtOptions {
  /** Connected App / External Client App consumer key. Defaults to $SF_CONSUMER_KEY / $consumer_key. */
  clientId?: string;
  /** Salesforce username to impersonate. Defaults to $SF_USERNAME / $salesforce_username. */
  username?: string;
  /** Treat the org as a sandbox (login at test.salesforce.com). Defaults to $SF_SANDBOX === "true". */
  sandbox?: boolean;
  /** Explicit audience / login URL; overrides `sandbox` and the env vars. */
  customUrl?: string;
  /** PEM private key contents. Defaults to the file at $SF_PRIVATE_KEY_PATH or ./server.key. */
  privateKey?: string;
  /** Timeout (ms) for the token exchange request. Defaults to 10000. */
  timeout?: number;
}
...
export function getSalesforceHomeUrl(): string {
...
export function createJwtAssertion(
  options: JwtOptions = {},
): { assertion: string; loginUrl: string } {
...
export async function jwtAuthenticate(options: JwtOptions = {}): Promise<SalesforceSession> {
...
async function verifyAuthenticatedUser({
  accessToken,
  identityUrl,
}: SalesforceSession): Promise<void> {
  if (!identityUrl) {
...
export function createFrontdoorUrl(
  session: SalesforceSession,
  redirectPath = "/lightning/page/home",
): string {
